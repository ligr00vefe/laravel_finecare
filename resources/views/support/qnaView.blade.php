    ...
    <article id="contents" class="">
        ...
        <div class="contents-body m-top-20">
            ...
            <div class="btn-wrap m-top-20">
                ...
                <form action="{{ route("support.qna.edit", [ "id" => $id->id ]) }}">
                    <button class="btn-cancel">수정</button>
                </form>
                <form action="{{ route("support.qna.destroy", [ "id" => $id->id ]) }}" onsubmit="if(!confirm('삭제하시겠습니까?')) { return false; }" method="post">
                    @csrf
                    @method("delete")
                    <button class="btn-cancel">삭제</button>
                </form>
                <a href="/support/qna/list" class="btn-black">목록</a>
            </div>
