...
@section("content")
    <section id="faq-view" class="wrapper">
        ...
        <div class="form-button-wrap">
            ...
            <form action="/admin/board/video/{{ $post->id }}" method="post" onsubmit="if (!confirm('삭제하시겠습니까?')) { return false; }" style="display: inline-block">
                @csrf
                @method("delete")
                <input type="hidden" name="id" value="{{ $post->id ?? "" }}">
                <div class="form-button-div"><button class="delete-link form-button-link">삭제</button></div>
            </form>
            <div class="form-button-div"><a class="modify-link form-button-link" href="/admin/board/video/edit/{{ $post->id }}">수정</a></div>
            <div class="form-button-div"><a class="write-link form-button-link" href="/admin/board/video/write">글쓰기</a></div>
        </div>
    </section>
@endsection
